...
4. 不要用一个原始指针初始化多个shared_ptr；
```c++
int *p1 = new int;
shared_ptr<int> p2(p1);
shared_ptr<int> p3(p1); // error
```
5. 不要在函数实参中创建`shared_ptr`;
```c++
...
6. 通过`shared_from_this()` 返回`this`指针，不要将`this`指针作为`shared_ptr` 返回出来，因为`this`指针本质上是一个裸指针，因此，这样可能导致重复析构。事例代码如下：
```c++
#include <iostream>
#include <memory>
...
8. C++17中的shared_ptr才支持数组
9. unique_ptr需要确定删除器的类型，所以不能像shared_ptr那样直接指定删除器，得这样写：
```c++
...
```
10. 应用需求不同：如果希望只有一个智能指针管理资源或者管理数组就用unique_ptr，如果希望多个智能指针管理同一个资源就用shared_ptr。
...
11. 通过use_count()方法获取当前观察资源的引用计数，如下所示：
```c++
shared_ptr<int> sp(new int(10));
weak_ptr<int> wp(sp);
cout << wp.use_count() << endl; //结果讲输出1
```
12. 通过expired()方法判断所观察资源是否已经释放，如:
```c++
shared_ptr<int> sp(new int(10));
weak_ptr<int> wp(sp);
if(wp.expired())
