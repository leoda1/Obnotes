[huggingface.co](https://huggingface.co/spaces/nanotron/ultrascale-playbook?section=one_forward,_one_backward_and_llama_3.1_schemes)
回顾TP当中，我们将tensor并行性的规模超过一个node上的4或者8个gpu的时候会迫使我们用低带宽lower-bandwidth的网络通讯，简言之走了net（机间的IB，roce等）。

![256MB数据在单机到64机（8gpu / node）上allreduce，algather，reduce-scatter的bandwidth变化图](attachment:e87f7c31-e0d8-4bcf-b008-a5b6a10e46e2:image.png)

256MB数据在单机到64机（8gpu / node）上allreduce，algather，reduce-scatter的bandwidth变化图
...
![8B model使用PP前后对比](attachment:7aa1eee1-a393-4d7c-a4c0-ee5931d45543:image.png)
...
![模型的PP并行示例，参数：16个layer 4个GPU](attachment:87844f8f-0e22-46f4-a169-3342aafe744d:image.png)

模型的PP并行示例，参数：16个layer 4个GPU
...
![1F1B调度示意图](attachment:8ce5ae55-b80d-4683-8606-9e0fb2d69c8b:image.png)
...
- 1F1B PP implementation in Picotron
...
![1F1B在集群上的benchmarks](attachment:16bae393-8b2a-4611-99bd-6e65ca69201a:image.png)
...
![4GPU的interleaved pipeline，图中模型第一层和最后一层是不同颜色的](attachment:e71fe010-f07f-4b9a-9e51-b6145f656021:image.png)

4GPU的interleaved pipeline，图中模型第一层和最后一层是不同颜色的
...
![image.png](attachment:9b42d07e-63ee-4bb1-9cd0-68edf1a0f1b9:image.png)
...
选择的详细解释见论文：https://arxiv.org/pdf/2211.05953

**Llama 3.1 所采用的流水线并行策略**所需的全部要素就是以上的部分，采用的是1F1B调度+IS交错阶段，并支持广度和深度优先之间调节。

![image.png](attachment:d75d40eb-4c09-441a-b3a2-4fe0063fe51c:image.png)

接着，PP的新方法（DeepSeek方案https://arxiv.org/pdf/2412.19437）尝试将 bubble **几乎降为零；**

### 4. **Zero bubble and DualPipe**

> 对涉及的计算操作进行**更细粒度的划分**，从而以**最有效的方式进行交错调度（interleaving）**这个的灵感是来源于Sea AI Lab 提出的 Zero Bubble 技术https://arxiv.org/pdf/2401.10241，促成了DualPipe。
> 
