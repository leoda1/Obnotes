...
|      loadRemoteConnInfo()      |         拿到 NIXL 传下来的 ip/port/gpu_dix 后用`uccl_engine_connect()`建链，建完调用u`ccl_engine_start_listener()`，然后把 remote_agent -> uccl_conn_t* 记到 connected_agents_。         |
|           connect()            |                                                                                 \                                                                                  |
...
|           postXfer（）           |                                  走uccl_engine_read_vector()或者uccl_engine_write_vector()读写数据，返回当前的 transfer_id 写到 `nixlUcclReqH`。                                   |
|          checkXfer()           |                          从nixlUcclReqH 拿出当前要等的 transfer id, 调用 `uccl_engine_xfer_status`  去轮询，如果需要 notif 的话就要多一个 `uccl_engine_send_notif`                          |
|         releaseReqH()          |                                                                                 \                                                                                  |
